...
```shell
yarn workspace web add @emotion/babel-plugin-jsx-pragmatic babel-plugin-macros
```

Twin and Emotion

```shell
yarn workspace web add twin.macro @emotion/react @emotion/styled
```
...
You can add Twin’s `GlobalStyles` import in `web/src/App.js`:
...
import { GlobalStyles } from 'twin.macro' // Import Global Styles

import './index.css'

const App = () => (
  <>
    <GlobalStyles /> // place global styles in App
...
You can avoid adding the pragma yourself with the following babel config `web/.babelrc.js`:
...
```bash
yarn workspace web add @types/react -D
```
...
To fix this, create a `twin.d.ts` file in your project root (src/types/twin.d.ts` with create-react-app) and add these declarations:

```typescript
// web/src/types/twin.d.ts
import 'twin.macro'
